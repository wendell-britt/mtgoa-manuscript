...
The village still looked at what was happening, after a fashion: it read the org chart and called the chart a system. The village still explained the breakages, and built explanations that flattered whoever commissioned them. It still intervened, aiming the interventions at people's behaviour rather than at whatever made that behaviour the easy option. The village still shipped, and read every failure as a people problem, because a failed design informs only somebody willing to be wrong. It never gave the work away. Building is where the Architect stays visible, and nobody leaves the one spot that proves the village still needs them.
