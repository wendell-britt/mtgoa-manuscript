# Copyright

Copyright © 2026 by Wendell Britt

All rights reserved. No part of this book may be reproduced in any form without
written permission from the author, except by a reviewer quoting brief passages in
a review.

Published by Wendell Britt

First edition, 2026

Cover and interior design by Wendell Britt

## Sources and permissions

This book is a synthesis. Where a framework here belongs to somebody else, it is
named in the text and mapped in full in Appendix G.

The developmental architecture underneath the Six Faces — and the
transcend-and-include principle that gives the book its structure — is **Ken
Wilber's**, from *Integral Life Practice* (with Terry Patten, Adam Leonard, and
Marco Morelli) and the broader Integral model, which builds on the Spiral
Dynamics work of **Clare Graves** and **Don Beck**.

The 3-2-1 Shadow Process in Appendix E is Wilber's, from *Integral Life Practice*.

The Five-Move Form in this book — Wake Up, Open Up, Clean Up, Grow Up, Show Up — adapts Ken
Wilber's five practices from *Finding Radical Wholeness*.

The practice of speaking as each interior voice, hearing its logic, and giving it
a narrower job comes from the **Big Mind Process** of **Dennis Genpo Merzel**
(Genpo Roshi).

The approach to shadow — that a disowned pattern is doing a job for you rather
than defecting from you — is **Carolyn Elliott's**, from *Existential Kink*.

The three-factor account of trust in Chapter 1 — ability, benevolence and integrity —
is **Roger Mayer, James Davis and David Schoorman's**, from "An Integrative Model of
Organizational Trust."

The Genius Demon Ally, which is where the daemons in Chapter 2 come from, is
**Robin Rice's**.

*Leverage points*, used throughout Chapter 6, is **Donella Meadows'** term of art,
from *Thinking in Systems* and "Leverage Points: Places to Intervene in a System."

*Felt sense*, used in Chapters 1, 3, 5, 8, 9 and in Appendix A, is **Eugene
Gendlin's**, from *Focusing*.

The five emotional channels are a remix of **wu xing**, the five-phase system of
classical Chinese medicine. Chapter 3 says what the remix changed and Appendix G
points at the original.

The three emotional-body practices in Appendix D are drawn from the author's
*Igniting Joy*.

## A note on what this book is not

This book is a practice, not a treatment. It is not psychotherapy, medical
advice, or a substitute for either. The shadow work in Chapters 2 through 4 can
surface material heavy enough that you want a trained person in the chair with you. If it does,
that is the practice working, so getting help is the next move rather than a
failure of it.

Names and identifying details in the examples have been changed.

masteringallyship.com
