...
- **The production pipeline — added 2026-08-01. `instruments/book/README.md` is the brief.** `build_book.py` answers *is the book complete*; this answers *can it be set*, and they are different questions. `typeset.py` is the shared layer both formats read: it turns the five frame devices into styled classes, normalises the four chapter heading styles for display, and resolves the 251 `---` rules into the 61 that are scene breaks and the 190 that are not. `build_pdf.py` renders a 6x9 print interior through Typst; `build_epub.py` renders the ebook. Neither edits `manuscript/`, and both refuse to ship an artifact that fails its checks — chapters landing on a verso, a running head printed on a blank leaf, a character the interior font cannot set, a frame that did not survive conversion. Dependencies are in `instruments/requirements.txt`; `build/` is gitignored.
