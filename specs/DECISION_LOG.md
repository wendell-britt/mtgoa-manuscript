...
| **DL-22** | six shipping appendices — A, B, C, D, E, F | **Internal production metadata prints to the reader.** `Status:`, `Authority:` and `Location in book:` blocks sit above the first line of six of the seven shipping appendices, and `build_book.py` copies them into the deliverable verbatim. A reader opening Appendix B meets **"Status: Draft"** and a repo path before a word of the book. | `SPEC_REMOVE_APP_V1_2026-07-31.md` §7 measured 15 such lines reaching `build/MTGOA_PRINT_2026-07-31.md`. `placeholders.py` does not match this shape, which is why nothing has flagged it. | **Strip every block. Ruled 2026-08-01.** | The appendices open on their own first sentence. | **RULED, not yet applied.** Mechanical; needs a `placeholders.py` rule so the shape cannot return. |
