...
| **DL-22** | 33 sites in ch1–ch9, listed in full in `editorial_reports/2026-08-01/PASS3_LINE_EDITOR.md` §4 | **Readability defects that survive whatever the open structural rulings decide** — a pronoun with no antecedent, a modifier attached to the wrong clause, an instruction written as a description, one sentence printed twice verbatim. | Pass 3 · Line Editor scan against the merged manuscript at `ee18459`. `instruments/line_scan.py` (new) returns 186 candidates on six rules; reader adjudication plus every carried-over LINE and BLOCKED finding from 2026-07-31 reduces that to 33 flags. 16 July line findings verified **closed by edit**; CH7 L2 withdrawn as a false flag — the repeated formula is the chapter's five-section template. | **Diagnosis only. Nothing applied, no file in `manuscript/` touched.** Each flag carries a minimal proposed edit, the reader problem it solves, its risk to voice, and the case for leaving it alone. Wendell picks which become work. | A reader stops doubling back for what the sentence should have handed her, and the voice is not smoothed to get it. | **OPEN — awaiting Wendell's selection.** Not a print blocker; explicitly ranked below DL-20 for the ship window. |
