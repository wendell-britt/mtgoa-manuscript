...
| ID | Location | Reader problem | Evidence | Decision | Intended result | Status |
|---|---|---|---|---|---|---|
| **P0** | `ch1:269` only — **2 of 3 closed 2026-07-31** | **Three placeholders print verbatim, including an authoring note addressed to Wendell** | `[[TESTIMONY SLOT — WENDELL…]]`, `*[visual: the six Game Masters]*`, `**[ URL / QR ]**`; all verified in `build/MTGOA_PRINT_2026-07-31.md`. `gate.py` passes clean — it has no placeholder rule | ch7 testimony slot **CUT** (ruled — writing it would be fabrication); ch1 visual marker **CUT**; **ch1 app CTA DEFERRED by Wendell, links still being set up** | Nothing addressed to the author reaches a reader | **STILL BLOCKS PRINT — 1 site.** `placeholders.py` exits non-zero until the URL lands. Do not allowlist |
