...
### Rate, measured — no verdict attached

The estimate that followed the first test ("a few days, more than remains before
August 1") extrapolated from one section and is withdrawn. What is measured:

| | edits | effect |
|---|---|---|
| ch8 Section 2, 1,145 w | 18 | **2.18× → 1.50×** on that section |
| ch8, rest of chapter | 15 | 2.32× → 2.27× **chapter-wide** |
| ch8 total for the day | 33 | **2.38× → 2.27×** chapter-wide |

**A big local win barely moves a chapter.** Section 2 dropped 0.68× and is 9% of
ch8, so the chapter moved 0.11× across 33 edits. Chapter-level movement needs
every section, not a sample of the worst sentences.

From that: **~180 edits to take ch8 to roughly 1.6×**, on the order of **1,600
book-wide.** Batches this session ran 14, 15, 18, 22, 29 and 31 edits, so 15–30
per pass — **6–12 passes per chapter, 60–100 for the book.**

**Whether that fits before August 1 is a scheduling question and this spec does
not answer it.** Two facts bear on it: the work parallelises across chapters, and
every pass is independently shippable, so stopping halfway leaves the book better
rather than half-edited.

### Calibration point — approved

**ch8 Section 2 at 1.50× was read and approved: "It still sounds like the book.
Much improved."**

That settles what the numbers could not. **1.50× preserves voice**, and the
method that reached it — Lanham on the frames, figures left alone — is validated
rather than assumed. Later passes aim at the same mark and get checked the same
way: numbers first, then read it.

What that pass deliberately did not touch, and nothing else should: the fragment
cadences (*Who sees everything. Who can name every game*), the anaphora (*which
means… which means…*), deliberate repetition (*The design was elegant. The horse
was not thirsty*), and parallel definitions (*Redesigning the system is the
Architect*). Those are figures. Register work removes drift and leaves rhetoric
standing.

One edit in that pass was a regression and is corrected: *"Nobody names the
obvious, **which is that** the competition runs for real"* traded a copula for a
`which is` tail, already a 76-instance problem here. Trading one tic for another
is not a win, and no register check can see it — only the reading can.

**Priority order:** ch8, ch9, ch6, ch7, ch3, ch4, ch5, ch2, ch1.
...
- **~33 true expletive openers.** An earlier count said 144. Wrong: 118 were
  *It is/was* with a clear antecedent in the previous sentence — an ordinary
  pronoun subject, not a defect. Only 26 *There is/was* plus about 7 dummy
  *It is* are genuine. More evidence that the 86% ordinary-copula finding is the
  whole story.
